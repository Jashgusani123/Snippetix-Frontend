...
const LandingMain = () => {
  const [codeOwner, setCodeOwner] = useState<string>('Developer');
  const [isEditing, setIsEditing] = useState(false);
  const [tempValue, setTempValue] = useState(codeOwner);

  const handleDoubleClick = () => {
    setTempValue(codeOwner);
    setIsEditing(true);
  };

  const handleBlur = () => {
    setCodeOwner(tempValue || 'Developer');
    setIsEditing(false);
  };
        ...
        >
          ...
          <pre className="text-sm font-mono whitespace-pre-wrap mt-8">
            <code>
              ...
              <span className="text-teal-400">log</span>(greet(
              <span className="text-green-400">
                {isEditing ? (
                  <input
                    ...
                    onBlur={handleBlur}
                    autoFocus
                    className="bg-transparent border-b border-green-400 outline-none w-20"
                  />
                ) : (
                  <span onDoubleClick={handleDoubleClick}>"{codeOwner}"</span>
                )}
              </span>
              ))
            </code>
